...
- (UIImage *)croppedImage
{
    ...
    /*如果不把起始点设为整数,截出的图片(宽、高是固定的,这里是640 * 470)的宽、高会出现多加一个像素的情况。原因:起始点可能出现12.36这种情况,截图时系统可能进行了四舍五入,结果起始点变为12.0,而宽(高)则加上一个像素*/
    CGRect zoomedCropRect = CGRectMake((int)(cropRect.origin.x / ratio),
                                       (int)(cropRect.origin.y / ratio),
    ...
    /*将图片缩放成这个尺寸640 * 470 */
    UIImage *tempImage = [self originImage:image scaleToSize:CGSizeMake(640.0, 470.0)];
    image = tempImage;
...
{
#ifdef DEBUG
    NSLog(@"===originImage:%f,%f===", image.size.width, image.size.height);
#endif
