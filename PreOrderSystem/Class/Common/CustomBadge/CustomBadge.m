...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
	...
	CGSize stringSize = [badgeString sizeWithFont:[UIFont boldSystemFontOfSize:16]];
	CGFloat flexSpace;
	if ([badgeString length]>=2) {
		flexSpace = [badgeString length];
		rectWidth = 30 + (stringSize.width + flexSpace);
        rectHeight = 30;
		retValue = CGSizeMake(rectWidth*badgeScaleFactor, rectHeight*badgeScaleFactor);
	} else {
		retValue = CGSizeMake(30*badgeScaleFactor, 30*badgeScaleFactor);
	}
...
- (void)drawRect:(CGRect)rect {
	...
	if ([self.badgeText length]>0) {
		[badgeTextColor set];
        CGFloat sizeOfFont = 16.5*badgeScaleFactor;
        /*
